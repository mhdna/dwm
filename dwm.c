...
void focusstack(const Arg *arg) {
  ...
  // don't crash when stack is empty, pressing Mod-j/k...
  if (!selmon->sel)
    return;
...
void runautostart(void) {
  ...
  system("cd ~/.local/share/dwm; sleep 2s &&  ./autostart.sh &");
...
//		return;
//
//	if (c->fakefullscreen != 1 && c->isfullscreen) { // exit fullscreen -->
//fake fullscreen 		c->fakefullscreen = 2; 		setfullscreen(c, 0); 	} else if
//(c->fakefullscreen == 1) { 		setfullscreen(c, 0); 		c->fakefullscreen = 0; 	} else
//{ 		c->fakefullscreen = 1; 		setfullscreen(c, 1);
//	}
// }
...
void toggletag(const Arg *arg) {
  ...
  /* Note that this patch is made for dwm 6.2. If applying this on top of the
   * latest master then the https://dwm.suckless.org/patches/alwaysfullscreen/
   * patch has been merged upstream, which prevents focus to drift from windows
   * that are in fake fullscreen. To address this issue use this if statement
   * instead: if (!selmon->sel || (selmon->sel->isfullscreen &&
...
/* 	rh = m->wh - 2*oh - ih * (((n - m->nmaster) / 2) - ((n - m->nmaster) %
 * 2 ? 0 : 1)); */
...
/* 			if ((i - m->nmaster) % 2 ) { */
/* 				resize(c, lx, ly, lw - (2*c->bw), (lh / lfacts) + ((i
 * - 2*m->nmaster) < 2*lrest ? 1 : 0) - (2*c->bw), 0); */
/* 				ly += HEIGHT(c) + ih; */
/* 			} else { */
/* 				resize(c, rx, ry, rw - (2*c->bw), (rh / rfacts) + ((i
 * - 2*m->nmaster) < 2*rrest ? 1 : 0) - (2*c->bw), 0); */
/* 				ry += HEIGHT(c) + ih; */
/* 			} */
/* 		} */
/* 	} */
 ...
 * of the screen *\/ */
/* 			resize(c, mx, my, (mw / mfacts) + (i < mrest ? 1 : 0) -
 * (2*c->bw), mh - (2*c->bw), 0); */
/* 			mx += WIDTH(c) + iv*mivf; */
/* 		} else { */
/* 			/\* stack clients are stacked horizontally *\/ */
/* 			resize(c, sx, sy, (sw / sfacts) + ((i - m->nmaster) < srest
